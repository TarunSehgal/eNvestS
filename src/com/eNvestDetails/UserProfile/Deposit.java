...
public class Deposit extends UserProfileDataElement {
	...
	@Override
	public void calculateDataelement(TransactionDetail transaction,
			String categoryHierarchy,UserProfileFactory profileFactory) {
		...
		try {
			...
			for (String captureDTO : captureElement) {
				...
				if(isAllowedCategory(transaction.getCategoryId(), dto.getPlaidCategory())){
					...
					dto.setEmployer(transaction.getName());	
				}			
			}
		} catch (Exception e) {
			e.printStackTrace();
